...
from selenium.common.exceptions import TimeoutException, ElementNotVisibleException, NoSuchElementException, \
    ElementNotSelectableException, UnexpectedAlertPresentException
...
def teardown_driver(driver_, display_, display_mode):
    not_closed, count = True, 0
    while (count < 10) and (not_closed is True):
        count += 1
        try:
            driver_.quit()
            not_closed = False
        except Exception as e:
            print "Encountered exception ", str(e), ", Retrying driver teardown (", count, " of 10 attempts)"
            print driver_.get_log('driver')
            sys.stdout.flush()
    try:
        if display_mode == 0:
            display_.stop()
        ...
        sys.stdout.flush()
    print "Driver killed after ", count, " attempts."
    sys.stdout.flush()
...
def abort_load(driver, display, tor, tor_call, display_mode, capture_path, process_tag, port, exits):
    ...
    while not_closed:
        try:
            ...
            not_closed = True
            print "Encountered a CSR Exception. "
            print "Could not kill the current driver. Trying again..."
            sys.stdout.flush()
...
def save_screenshot(driver, filename, path):
    ...
    done, csr, count = False, False, 0
    while (done is False) and (count < 10):
        count += 1
        try:
            ...
            print "Encountered a CSR exception"
            print "Could not complete the screenshot. Trying again..."
            sys.stdout.flush()
            time.sleep(1)
        except (TimeoutError, TimeoutException):
            ...
            print "Encountered an exception: ", str(ioe)
            sys.stdout.flush()
            return "IOError"
        except Exception as e:
            if csr is False:
                ...
                return "Exception"
    if count >= 10:
        print "Failed to save screenshot after 10 attempts"
        sys.stdout.flush()
        return "Failed"
    return "No Error"
...
def save_html(driver, filename, path):
    ...
    while (done is False) and (count < 10):
        ...
        try:
            ...
            print "Encountered a CSR exception"
            print "Could not save the html. Trying again..."
            sys.stdout.flush()
            time.sleep(1)
        except (TimeoutError, TimeoutException):
            ...
            if csr is False:
                ...
                return "Exception"
    if count >= 10:
        print "Failed to save html after 10 attempts"
        sys.stdout.flush()
        return "Failed"
    f.close()
    return "No Error"
...
def crawl_setup(tor=False, capture_path="", display_mode=0, port="9000", process_tag="1", exits="US"):
    ...
    if tor is True:
        ...
        tor_call = start_program("/usr/sbin/tor -f " + torrc_file, delay=60)
    else:
    ...
    logging.debug("Allowed log types: %s" % driver_.log_types)
    return driver_, display_, tor_call


def setup_profile(tor=False, port="9000", firebug=True, netexport=True, noscript=False, capture_path="",
                  cache_enabled=False):
    profile = webdriver.FirefoxProfile()
    profile.set_preference("app.update.enabled", False)
    profile.set_preference("webdriver.log.driver", "ALL")
    profile.set_preference("webdriver.log.driver.level", "ALL")
    profile.set_preference("webdriver.log.browser.level", "ALL")
    if tor is True:
...
def load_page(driver, url, cookies=0):
    ...
    try:
        ...
        sys.stdout.flush()
    except (TimeoutException, TimeoutError, UnexpectedAlertPresentException) as te:
        logging.error("Exception occurred while loading page: %s" % url)
        sys.stdout.flush()
        return str(te)
    try:
        ...
        if element is not None:
            ...
            print "Alert not found on page: " + url
            # print driver.get_log('driver')
            raise NoAlertPresentException
    except (TimeoutException, NoAlertPresentException, UnexpectedAlertPresentException, BadStatusLine):
        # TODO: this info is annoying, consider logging this at debug level
        logging.debug("No alert found on page: %s" % url)
        sys.stdout.flush()
        pass
    ...
    try:
        ...
        print "Timeout / Popup Exception occurred while loading page: " + url
        sys.stdout.flush()
        return str(te)
    sys.stdout.flush()
    return "No Error"
...
def do_crawl(sites, driver, display, capture_path="", tor=False, tor_call="", search=False, search_log="",
    ...
    status_time = capture_path + "/time-" + process_tag
    ...
    for s in sites:
        ...
        str_status = "Loading URL: " + str(s[1]) + " \t Index: " + str(s[0]) + " \t " + str(status_code) + "\t Time: " + \
        ...
        if str(status_code) == "No Error":
            ...
            time.sleep(1)
            if (html_status == "No Error") and (ss_status == "No Error"):
                if search is True:
        ...
        if (str(status_code) != "No Error") or (str(ss_status) != "No Error") or (str(html_status) != "No Error"):
            driver, display = abort_load(driver, display, tor, tor_call, display_mode, capture_path, process_tag, port,
                                         exits)
            print str_status
            sys.stdout.flush()
...
def do_search(rules, driver, display, capture_path="", tor=False, tor_call="", display_mode=0, process_tag="1",
    ...
    for r in rules:
        ...
        str_status = "Loading URL: " + str(r['url']) + " \t Index: " + str(r['index']) + " \t " + str(
            status_code) + "\t Time: " + \
                     str(time.asctime(time.localtime())) + "\n"
        print str_status
        sys.stdout.flush()
...
def do_playback(credentials, playback_file, driver, display, capture_path="", process_tag="1", tor=False, tor_call="",
    ...
    for c in credentials:
        ...
        str_status = "Loading URL: " + str(c[1]) + " \t Index: " + str(c[0]) + " \t " + str(status_code) + "\t Time: " + \
                     str(time.asctime(time.localtime())) + "\n"
        print str_status
        sys.stdout.flush()
        sf = open(status_file, "w")
        ...
        if str(status_code) == "No Error":
            ...
            for a in actions:
                ...
                make_folder(capture_path)
                save_html(driver, save_name, capture_path + "/html/")
